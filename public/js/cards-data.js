 ...
 * - thumb  (240px)  → card trong tay người chơi
 * - medium (500px)  → card trên bàn chơi (slot)
 * - full   (800px)  → modal xem chi tiết
 * Fallback: img gốc PNG (nếu WebP chưa tải được)
 */
...
/**
 * Helper: lấy URL ảnh phù hợp theo kích thước.
 * Dùng trong render card HTML.
 * @param {object|string} img - card.img (object hoặc string legacy)
 * @param {'thumb'|'medium'|'full'} size
 * @returns {string} URL
 */
function getCardImgUrl(img, size) {
    if (!img) return '';
    // Legacy: img là string
    if (typeof img === 'string') return img;
    return img[size] || img.medium || img.original || '';
}
